        ...
        for (int i = 0; i < types.length; i++) {
            if (types[i] == AsyncResponse.class) {
                if (AnnotationUtils.getAnnotation(m.getParameterAnnotations()[i], Suspended.class) == null) {
                    reportInvalidResourceMethod(m, NOT_SUSPENDED_ASYNC_MESSAGE_ID, Level.FINE); //Liberty change - CXF-7121
                    return false;
                }
                if (m.getReturnType() == Void.TYPE || m.getReturnType() == Void.class) {
                    return true;
                } else {
                    reportInvalidResourceMethod(m, NO_VOID_RETURN_ASYNC_MESSAGE_ID, Level.WARNING); //Liberty change - CXF-7121
